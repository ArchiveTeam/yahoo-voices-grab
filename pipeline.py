...
class CheckIP(SimpleTask):
    ...
    def process(self, item):
        ...
        ip_str = socket.gethostbyname('voices.yahoo.com')

        if not ip_str.startswith('98.138.'):
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'yahoovoices.lua'))
...
project = Project(
    ...
    """,
    utc_deadline=datetime.datetime(2014, 7, 30, 23, 58, 0)
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="yahoovoices"),
    WgetDownload(
        WgetArgs(),
        max_tries=5,
